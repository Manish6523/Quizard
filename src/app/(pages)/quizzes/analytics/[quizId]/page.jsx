...
export default function AnalyticsPage() {
  ...
  const summaryStats = useMemo(() => {
    if (!analytics || !analytics.quiz_attempts || analytics.quiz_attempts.length === 0) {
      return {
        totalAttempts: 0,
        averageScore: 0,
    ...
    return {
      ...
      averageScore: Math.round((totalScore / attempts.length / totalQuestions) * 100),
  ...
  const questionBreakdown = useMemo(() => {
    if (!analytics || !analytics.quiz_attempts || !analytics.quizzes || analytics.quiz_attempts.length === 0) return [];
  ...
  const attemptsOverTime = useMemo(() => {
    ...
    analytics.quiz_attempts.forEach(attempt => {
        const date = new Date(attempt.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
        if (!attemptsByDate[date]) {
            attemptsByDate[date] = 0;
        }
        attemptsByDate[date]++;
    });
    return Object.entries(attemptsByDate).map(([date, attempts]) => ({ date, attempts }));
  }, [analytics]);

  // Memoized calculation for score distribution (Radial Chart)
  const scoreDistribution = useMemo(() => {
    if (!analytics || !analytics.quiz_attempts || analytics.quiz_attempts.length === 0) return [];
    const totalQuestions = analytics.quizzes.length;
    ...
    return [
        { name: 'Excellent (80%+)', value: distribution.excellent, fill: '#22c55e' },
        { name: 'Good (60-79%)', value: distribution.good, fill: '#f59e0b' },
        { name: 'Needs Improvement (<60%)', value: distribution.needsImprovement, fill: '#ef4444' },
    ];
  }, [analytics]);
    ...
    return (
      <div className="flex h-screen items-center justify-center p-4">
        <Card className="w-full max-w-md text-center">
          <CardHeader><CardTitle>Analytics Not Found</CardTitle></CardHeader>
          <CardContent><p>Could not load analytics for this quiz.</p></CardContent>
        </Card>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-8 mt-20">
      <header className="mb-8">
        ...
        <p className="text-muted-foreground text-lg mt-2">
          Results for <span className="font-semibold text-primary">{analytics.title}</span>
        </p>
      </header>

      {/* Summary Cards */}
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <StatCard icon={<Users />} title="Total Attempts" value={summaryStats.totalAttempts} />
        <StatCard icon={<Target />} title="Average Score" value={`${summaryStats.averageScore}%`} />
        <StatCard icon={<HelpCircle />} title="Total Questions" value={summaryStats.totalQuestions} />
      </div>
        ...
        <div className="lg:col-span-2 space-y-8">
            ...
            <Card className="shadow-sm">
                <CardHeader>
                    ...
                    <CardDescription>How many students took the quiz each day.</CardDescription>
                </CardHeader>
                <CardContent>
        ...
        <div className="space-y-8">
            ...
            <Card className="shadow-sm">
                <CardHeader>
                    ...
                    <CardDescription>Performance brackets across all attempts.</CardDescription>
                </CardHeader>
                ...
                <CardContent>
                    <Table>
                        ...
                        <TableBody>
                            ...
                            <TableRow key={index}>
                                <TableCell>
                                <div className="font-medium">{attempt.participant_name}</div>
